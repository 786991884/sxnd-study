...
public class BookServlet extends HttpServlet {
	...
	protected void cash(HttpServletRequest request, HttpServletResponse response)
		...
		// 1简单验证:验证表单域的值是否符合基本的规范：是否为空，是否可以转为int类型，是否是一个email.不需要进行查询
		// 数据库或调用任何的业务方法
		...
		// 验证通过执行具体的逻辑操作
		bookService.cash(BookStoreWebUtils.getShoppingCart(request), username,
				accountId);
		response.sendRedirect(request.getContextPath() + "/success.jsp");
	}

	// 验证余额是否充足
	public StringBuffer validateBalance(HttpServletRequest request,
										String accountId) {
	...
	protected void updateItemQuantity(HttpServletRequest request,
									  HttpServletResponse response) throws ServletException, IOException {
		// 4在updateItemQuantity方法中，获取quantity,id,再获取购物车对象，调用service的方法做修改
		...
		// 5传回JSON数据:bookNumber:xx,totalMoney
	...
	protected void addToCart(HttpServletRequest request,
		...
		if (id > 0) {
			// 2获取购物车对象
			ShoppingCart sc = BookStoreWebUtils.getShoppingCart(request);
			// 3调用BookService的addtoCart方法把商品放到购物车中
