...
<script type="text/javascript">
	$(function() {
		$(".delete").click(function() {
			...
			var flag = confirm("确定在删除" + title + "的信息吗?");
			if (flag) {
				return true;
			}
			return false;
		...
		//ajax修改单个商品的数量
		//1.获取页面中所有的text，并为其添加onchange响应函数。弹出确认对话框：确定要修改吗
		$(":text").change(function() {
			...
			var title = $.trim($tr.find("td:first").text());
			if (quantity == 0) {
				var falg2 = confirm("确定要删除" + title + "吗?");
				if (falg2) {
					//得到了a节点
			...
			$.post(url, args, function(data) {
				...
				$("#bookNumber").text("你的购物车中共有" + bookNumber + "本书");
			}, "JSON");
		});
	});
</script>
...
<body>
	<center>
		<br> <br>
		<div id="bookNumber">您的购物车中共有
			${sessionScope.ShoppingCart.bookNumber }本书</div>
		<table cellpadding="10">
			...
			<c:forEach items="${sessinoScope.ShoppingCart.items }" var="item">
				<tr>
			...
			<tr>
				<td colspan="4"><a
					...
					<a href="bookServlet?method=clear">清空购物车</a> &nbsp;&nbsp; <a
					href="bookServlet?method=forwardPage&page=cash">结账</a> &nbsp;&nbsp;
				</td>
			</tr>
		</table>
