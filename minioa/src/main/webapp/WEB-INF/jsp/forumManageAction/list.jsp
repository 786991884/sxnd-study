	...
	<div id="MainArea">
			...
			<tbody id="TableData" class="dataContainer" datakey="forumList">
				<s:iterator value="#forumList" status="status">
					<tr class="TableDetail1 template">
						<td>${name }&nbsp;</td>
						<td>${description }&nbsp;</td>
						<td><s:a action="forumManageAction_delete?id=%{id}" onclick="return delConfirm()">删除</s:a> <s:a action="forumManageAction_editUI?id=%{id}">修改</s:a> <!-- 最上面的不能用上移 --> <s:if test="#status.first">
								<span class="disabledA">上移</span>
							</s:if> <s:else>
								<s:a action="forumManageAction_moveUp?id=%{id}">上移</s:a>
							</s:else> <!-- 最下面的不能用下移 --> <s:if test="#status.last">
		...
		<div id="TableTail">
			<div id="TableTail_inside">
				<s:a action="forumManageAction_addUI">
